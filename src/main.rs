...
fn main()
{
    ...
    let mut plr_weapons: HashMap<String, u32> = HashMap::new();
    plr_weapons.insert(weapon_name, weapon_dmg);
    
    let basic_armor: Armor = Armor::new("BASIC".to_string(), 10, 1, 100)
        .unwrap();
    
    let mut main_plr: Player = Player::new(player_name, 100, plr_weapons, &basic_armor);
    ...
    let mut enemy_weapons: HashMap<String, u32> = HashMap::new();
    enemy_weapons.insert("Sword".to_string(), 10);
    
    let mut enemy: Player = Player::new("Enemy".to_string(), 50, enemy_weapons, &basic_armor);
    ...
    drop(weapon_use);
    let winner = plr_man.get_winner().unwrap();
